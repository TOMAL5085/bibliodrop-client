...
import { countUsers, listTransactions } from "@/lib/persistence";
...
function getRevenueTrend(transactions: Awaited<ReturnType<typeof listTransactions>>) {
  return monthLabels.map((name, monthIndex) => {
    const monthTransactions = transactions.filter((transaction) => getMonthIndex(transaction.date) === monthIndex);
...
function getCategoryBreakdown() {
  return Object.entries(
    books.reduce<Record<string, number>>((accumulator, book) => {
...
export default async function AdminDashboardPage() {
  ...
  const totalRevenue = liveTransactions.reduce((sum, transaction) => sum + transaction.amount, 0);
  const revenueTrend = getRevenueTrend(liveTransactions);
  const categoryBreakdown = getCategoryBreakdown();
  const approvalQueue = books.filter((book) => book.status === "pending_approval");
